...
var historise_etat_fixe={id:97,nom:"historique_etat_fixe_modif",etat:"OFF",lastrun:null};
var timer=null;
...
function Histo(){
	var timestamp=req.moment().format('YYYY-MM-DD HH')+':59:00';
	...
	var date_pour_suppr = req.moment().weekday(-21).format('YYYY-MM-DD')+' 23:59:00'
	                ...
	                if (rows[0]) {
	                	...
	                	for (var l in rows){
							...
							if (periph && periph.last_etat && periph.last_etat.expression) {
								...
								var dataarray_heure=[date_sql+":59:00",uuid,
											...
											if (rows[0]) {
												...
												for (var l in rows){
													...
													if (periph && periph.last_etat && periph.last_etat.expression){
														...
														var dataarray_jour=[date_sql+" 23:59:00",uuid,
																    periph.last_etat.expression.etat,periph.last_etat.expression.expr1,
																    periph.last_etat.expression.expr2,periph.last_etat.expression.expr3,
																    periph.last_etat.expression.expr4,periph.last_etat.expression.expr5,
																    periph.last_etat.expression.expr6,periph.last_etat.expression.expr7,
																	...
																	if (rows[0]) {
																		...
																		for (var l in rows){
																			...
																			if (periph && periph.last_etat && periph.last_etat.expression){
																				...
																				var dataarray_mois=[date_sql+"-"+jour_fin_mois+" 23:59:00",uuid,
																						    periph.last_etat.expression.etat,periph.last_etat.expression.expr1,
																						    periph.last_etat.expression.expr2,periph.last_etat.expression.expr3,
																						    periph.last_etat.expression.expr4,periph.last_etat.expression.expr5,
																						    periph.last_etat.expression.expr6,periph.last_etat.expression.expr7,
																							...
																							if (rows[0]) {
																								...
																								for (var l in rows){
																									...
																									if (periph && periph.last_etat && periph.last_etat.expression){
																										...
																										var dataarray_an=[date_sql+"-12-31 23:59:00",uuid,
																												    periph.last_etat.expression.etat,periph.last_etat.expression.expr1,
																												    periph.last_etat.expression.expr2,periph.last_etat.expression.expr3,
																												    periph.last_etat.expression.expr4,periph.last_etat.expression.expr5,
																												    periph.last_etat.expression.expr6,periph.last_etat.expression.expr7,
																								...
																								},historise_etat_fixe.fileattentedata,'histo');
																							}
																						},null,'histo');
////-------------------															
																			//}
...
select  h.uuid, max(expr1*1), max(expr1*1) expr1_max, min(expr1*1) expr1_min, round(avg(expr1*1),2) expr1_moy 
							...
							max(expr6_unit), max(expr7_unit),strftime('%Y-%m-%d %H',h.timestamp)||':59:00'
							from 
							  historique  h  

group by h.uuid,strftime('%Y-%m-%d %H',h.timestamp)||':59:00';



delete * from historique_jour;
...
select  h.uuid, max(expr1*1), max(expr1*1) expr1_max, min(expr1*1) expr1_min, round(avg(expr1*1),2) expr1_moy 
							...
							, max(etat*1), max(etat*1) etat_max, min(etat*1) etat_min, round(avg(etat*1),2) etat_moy 
							, max(etat_unit), max(expr1_unit) , max(expr2_unit), max(expr3_unit) ,max(expr4_unit) , max(expr5_unit), 
							max(expr6_unit), max(expr7_unit),strftime('%Y-%m-%d',h.timestamp)||' 23:59:00'
							from  historique  h   
group by h.uuid,strftime('%Y-%m-%d',h.timestamp)||' 23:59:00';



delete * from historique_mois;
...
select  h.uuid, max(expr1*1), max(expr1*1) expr1_max, min(expr1*1) expr1_min, round(avg(expr1*1),2) expr1_moy 
							...
							, max(etat*1), max(etat*1) etat_max, min(etat*1) etat_min, round(avg(etat*1),2) etat_moy 
							, max(etat_unit), max(expr1_unit) , max(expr2_unit), max(expr3_unit) ,max(expr4_unit) , max(expr5_unit), 
							max(expr6_unit), max(expr7_unit),strftime('%Y-%m-',h.timestamp)||strftime('%d',date(h.timestamp,'start of month','+1 month','-1 day'))||' 23:59:00'
							from  historique  h 
group by h.uuid,strftime('%Y-%m-',h.timestamp)||strftime('%d',date(h.timestamp,'start of month','+1 month','-1 day'))||' 23:59:00';
...
select  h.uuid, max(expr1*1), max(expr1*1) expr1_max, min(expr1*1) expr1_min, round(avg(expr1*1),2) expr1_moy 
							...
							, max(etat*1), max(etat*1) etat_max, min(etat*1) etat_min, round(avg(etat*1),2) etat_moy 
							, max(etat_unit), max(expr1_unit) , max(expr2_unit), max(expr3_unit) ,max(expr4_unit) , max(expr5_unit), 
							max(expr6_unit), max(expr7_unit),strftime('%Y',h.timestamp)||'-12-'||strftime('%d',date(h.timestamp,'start of month','+1 month','-1 day'))||' 23:59:00'
							from  historique  h 
group by h.uuid,strftime('%Y',h.timestamp)||'-12-31'||' 23:59:00';
 * 
 * 
 * 
 * */
